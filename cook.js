...
async function launchBrowserWithProxy(proxyDetails) {
    ...
    try {
        ...
        // const cookies = await loadCookiesFromFile('cookies.json');
        // await setCookies(page, cookies);

        
        if (proxyDetails.proxyUsername && proxyDetails.proxyPassword) {
            await page.authenticate({
                username: "dass1223-zone-resi-region-us-session-92ed0e67444a-sessTime-5",
                password: "Dassdass",
            });
        }
...
async function handleBrowserInstance(proxyConfig, instanceNumber) {
    ...
    try {
        ...
        if (!page.isClosed()) {
            ...
            // await retryOperation(() => safeNavigate(page, "https://multiplecontent.exblog.jp/"));
            // await humanLikeDelay(4000, 7000);

            // if (!page.isClosed()) {
            //     await page.bringToFront();
            //     const posts = [
            //         'a[href*="https://multiplecontent.exblog.jp/32637942/"]',
            //         'a[href*="https://multiplecontent.exblog.jp/32637936/"]',
            //         'a[href*="https://multiplecontent.exblog.jp/30559167/"]'
            //     ];
...
async function waitForSelectorWithTimeout(page, selector, timeout = 120000) {
    try {
...
async function retryScroll(page, direction, maxRetries = 3) {
    for (let i = 0; i < maxRetries; i++) {
        try {
            await humanLikeScroll(page, direction);
            return;
        } catch (error) {
            console.error(`Scrolling attempt ${i + 1} failed:`, error);
            if (i === maxRetries - 1) {
                console.error('Max retries reached for scrolling. Continuing...');
            } else {
...
async function handlePosts(page, posts, index) {
    ...
    try {
        await waitForSelectorWithTimeout(page, postSelector);
...
async function main() {
    while (true) {
        ...
        try {
            await Promise.all([
                handleBrowserInstanceWithRetry(proxies[0], 1).catch(e => console.error('Instance 1 error:', e)),
                handleBrowserInstanceWithRetry(proxies[1], 2).catch(e => console.error('Instance 2 error:', e)),
                handleBrowserInstanceWithRetry(proxies[2], 3).catch(e => console.error('Instance 3 error:', e))
            ]);
        } catch (error) {
